...
func (p *PartitionStateInProgress) NewRowsIterInCdc() *rowsIterInCdc {
	iter := p.rows.Copy().Iter()
	ret := &rowsIterInCdc{
		iter: iter,
	}
...
func (p *PartitionStateInProgress) NewObjectsIterInCdc() ObjectsIter {
...
func (p *PartitionStateInProgress) NewBlockDeltaIter() BlockDeltaIter {
	ret := &blockDeltaIter{}
	return ret
}
