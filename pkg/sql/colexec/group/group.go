...
func (ctr *container) initResultAndHashTable(proc *process.Process, config *Argument) (err error) {
	...
	switch {
	case ctr.keyWidth <= 8:
		if ctr.intHashMap, err = hashmap.NewIntHashMap(ctr.groupVecsNullable, config.Ibucket, config.Nbucket, proc.Mp()); err != nil {
			return err
		}
		if config.PreAllocSize > 0 {
	...
	default:
		if ctr.strHashMap, err = hashmap.NewStrMap(ctr.groupVecsNullable, config.Ibucket, config.Nbucket, proc.Mp()); err != nil {
			return err
		}
		if config.PreAllocSize > 0 {
