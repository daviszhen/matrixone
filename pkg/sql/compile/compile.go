...
func (c *Compile) SetTempEngine(tempEngine engine.Engine, tempStorage *memorystorage.Storage) {
	...
	if c.Ctx() != nil && c.Ctx().Value(defines.TemporaryTN{}) == nil {
		c.SetCtx(context.WithValue(c.Ctx(), defines.TemporaryTN{}, tempStorage))
	}
}

// Compile is the entrance of the compute-execute-layer.
// It generates a scope (logic pipeline) for a query plan.
func (c *Compile) Compile(_ context.Context, pn *plan.Plan, fill func(*batch.Batch) error) (err error) {
	start := time.Now()
	defer func() {
	...
	defer func() {
		if e := recover(); e != nil {
			...
			c.proc.Error(c.proc.Ctx, "panic in compile",
	...
	c.proc.Ctx = context.WithValue(c.proc.Ctx, defines.TestKillKey{}, []string{"Compile.Compile"})

	StartCheckRoutine(c.Ctx(), c.proc.TestKill, "enter Compile.Compile "+"---1111")

	// generate logic pipeline for query.
	c.scope, err = c.compileScope(c.proc.Ctx, pn)

	StartCheckRoutine(c.Ctx(), c.proc.TestKill, "enter Compile.Compile "+"---2222")
...
func StartCheckRoutine(ctx context.Context, testKill bool, tip string) {
	if ctx == nil || !testKill {
		return
	}
	go func() {
		fmt.Fprintln(os.Stderr, "==testkill", tip, "setup check*****")
		select {
		case <-ctx.Done():
			fmt.Fprintln(os.Stderr, "==testkill", tip, "ctx is Cancelled")
		}
	}()
}
...
func (c *Compile) Run(_ uint64) (result *util2.RunResult, err error) {
	...
	StartCheckRoutine(c.Ctx(), c.proc.TestKill, "enter Compile.Run "+"+++abc")

	sp := c.proc.GetStmtProfile()
	tempCtx, span = trace.Start(c.Ctx(), "Compile.Run", trace.WithKind(trace.SpanKindStatement))
	c.SetCtx(tempCtx)
	...
	if c.proc.TestKill {
		pAddr := fmt.Sprintf("%p", c.proc.Ctx)
		fmt.Fprintln(os.Stderr, "==testkill", "enter Compile.Run", "proc.Ctx", pAddr)

		StartCheckRoutine(c.proc.Ctx, c.proc.TestKill, "enter Compile.Run "+pAddr)

		defer func() {
			fmt.Fprintln(os.Stderr, "==testkill", "exit  Compile.Run", "proc.Ctx", pAddr)
		}()
	}
...
func (c *Compile) compileScope(_ context.Context, pn *plan.Plan) ([]*Scope, error) {
	start := time.Now()
	defer func() {
	...
	case *plan.Plan_Query:
		...
		scopes, err := c.compileQuery(c.Ctx(), qry.Query)
		if err != nil {
			return nil, err
		}
	...
	case *plan.Plan_Ddl:
		...
		case plan.DataDefinition_SHOW_DATABASES,
			...
			return c.compileQuery(c.Ctx(), pn.GetDdl().GetQuery())
			// 1、not supported: show arnings/errors/status/processlist
			// 2、show variables will not return query
			// 3、show create database/table need rewrite to create sql
		}
	}
	return nil, moerr.NewNYI(c.Ctx(), fmt.Sprintf("query '%s'", pn))
}
...
func (c *Compile) compileQuery(_ context.Context, qry *plan.Query) ([]*Scope, error) {
	var err error
	...
	for i := len(qry.Steps) - 1; i >= 0; i-- {
		var scopes []*Scope
		var scope *Scope
		scopes, err = c.compilePlanScope(c.Ctx(), int32(i), qry.Steps[i], qry.Nodes)
		if err != nil {
			return nil, err
		}
...
func (c *Compile) compilePlanScope(_ context.Context, step int32, curNodeIdx int32, ns []*plan.Node) ([]*Scope, error) {
	start := time.Now()
	defer func() {
	...
	case plan.Node_EXTERNAL_SCAN:
		...
		ss, err = c.compileExternScan(c.Ctx(), node)
		if err != nil {
			return nil, err
		}
	...
	case plan.Node_SOURCE_SCAN:
		ss, err = c.compileSourceScan(c.Ctx(), n)
		if err != nil {
			return nil, err
		}
	...
	case plan.Node_FILTER, plan.Node_PROJECT, plan.Node_PRE_DELETE:
		...
		ss, err = c.compilePlanScope(c.Ctx(), step, n.Children[0], ns)
		if err != nil {
			return nil, err
		}
	...
	case plan.Node_JOIN:
		curr := c.anal.curr
		c.setAnalyzeCurrent(nil, int(n.Children[0]))
		left, err = c.compilePlanScope(c.Ctx(), step, n.Children[0], ns)
		if err != nil {
			return nil, err
		}
		c.setAnalyzeCurrent(left, int(n.Children[1]))
		right, err = c.compilePlanScope(c.Ctx(), step, n.Children[1], ns)
		if err != nil {
			return nil, err
		}
		c.setAnalyzeCurrent(right, curr)
		ss = c.compileSort(n, c.compileJoin(c.Ctx(), n, ns[n.Children[0]], ns[n.Children[1]], left, right))
		return ss, nil
	case plan.Node_SORT:
		curr := c.anal.curr
		c.setAnalyzeCurrent(nil, int(n.Children[0]))
	...
	case plan.Node_SINK:
		...
		return ss, nil
	default:
		return nil, moerr.NewNYI(c.Ctx(), fmt.Sprintf("query '%s'", n))
	}
}
