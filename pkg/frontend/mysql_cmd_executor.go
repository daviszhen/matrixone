...
func (mce *MysqlCmdExecutor) handleDDl(ses *Session, stmt tree.Statement, epoch uint64) error {
	...
	case *tree.CreateDatabase:
		...
		needCreate := true
		if ddl.IfNotExists {
			_, err := ses.storage.Database(ctx, name, engine.Snapshot(txnHandler.GetTxn().GetCtx()))
			if err == nil { //exists
				needCreate = false
			}
		}
		if needCreate {
			err := ses.storage.Create(ctx, name, engine.Snapshot(txnHandler.GetTxn().GetCtx()))
			if err != nil {
				return err
			}
		}
	case *tree.DropDatabase:
		...
		if needDrop {
			err := ses.storage.Delete(ctx, name, engine.Snapshot(txnHandler.GetTxn().GetCtx()))
			if err != nil {
				return err
			}
		}
