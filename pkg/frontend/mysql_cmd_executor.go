...
func allocateMysqlResultSet(source *MysqlResultSet,rowCount int,columnCount int) *MysqlResultSet {
	mrsx := &MysqlResultSet{}
	mrsx.Columns = source.Columns
	mrsx.Name2Index = source.Name2Index
	mrsx.Data = make([][]interface{}, rowCount)
	for i := int64(0); i < int64(rowCount); i++ {
		mrsx.Data[i] = make([]interface{}, columnCount)
	}
	return mrsx
}
...
func getDataFromPipeline(obj interface{}, bat *batch.Batch) error {
	...
	err := protoImpl.lastNullPacket()
	if err != nil {
		logutil.Errorf("lastNullPacket is failed.error:%v",err)
		return err
	}
	...
	logutil.Infof("rowCount %v \n" +
		...
		tTime - row2colTime - protoImpl.makeTime - protoImpl.sendTime - allocateOutBufferTime,
		protoImpl.String())

	return nil
}

func getDataFromPipelineAsync(obj interface{}, bat *batch.Batch) error {
	rt := obj.(*Routine)
	ses := rt.GetSession()
	...
	if n := len(bat.Sels); n == 0 {
		...
		for j := 0; j < n; j++ { //row index
			...
			//allocate resultset
			mrsx := allocateMysqlResultSet(ses.Mrs,1,len(bat.Vecs))
			row := mrsx.Data[0]
	...
	err := rt.PushResultSet(nil)
	if err != nil {
		return err
	}

	tTime := time.Since(begin)
	logutil.Infof("time of getDataFromPipelineAsync : %s row2col %v restTime %v resultset %s",
		tTime,
		row2colTime,
		tTime - row2colTime - protoImpl.makeTime - protoImpl.sendTime,
		protoImpl.String())
