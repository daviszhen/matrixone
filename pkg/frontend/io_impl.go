...
func (len *LenEnc) Open(ctx context.Context, options *MysqlWritePacketOptions) error { return nil }

func (len *LenEnc) Write(ctx context.Context, out WriteBuffer, options *MysqlWritePacketOptions) (err error) {
	err = out.Open(ctx, nil)
	if err != nil {
		return err
	}

	pos := writeIntLenEnc(len.buf[:], 0, options.len)
	err = out.Write(ctx, len.buf[:pos], nil)
	if err != nil {
		return err
	}
...
func (len *LenEnc) Close(ctx context.Context) error {
	return nil
}

func (col *ColumnDef) Open(ctx context.Context, options *MysqlWritePacketOptions) error {
	return nil
}

func (col *ColumnDef) Write(ctx context.Context, out WriteBuffer, options *MysqlWritePacketOptions) (err error) {
	mysqlColumn, ok := options.column.(*MysqlColumn)
	if !ok {
		return moerr.NewInternalError(ctx, "sendColumn need MysqlColumn")
	}

	var data []byte
	if options.capability&CLIENT_PROTOCOL_41 != 0 {
...
func (format *MysqlRowWriter) Open(ctx context.Context, opts *WriterOptions) error {
...
func (format *MysqlRowWriter) writeRow(ctx context.Context, row []any, opts *WriterOptions) (err error) {
	if opts.isBinary {
...
func (format *MysqlRowWriter) Write(ctx context.Context, chunks Chunks, opts *WriterOptions) (err error) {
	opts.writeRow = format.writeRow
	return format.ChunksWriter.Write(ctx, chunks, opts)
}
func (format *MysqlRowWriter) WriteBytes(context.Context, []byte, *WriterOptions) error {
	return nil
}
func (format *MysqlRowWriter) Flush(context.Context) error {
	return nil
}
func (format *MysqlRowWriter) Close(ctx context.Context) error {
