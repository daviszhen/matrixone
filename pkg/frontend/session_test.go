...
=======
		...
		err = ses.GetTxnHandler().SetAutocommit(false, true)
		convey.So(err, convey.ShouldBeNil)

		err = ses.GetTxnHandler().SetAutocommit(false, false)
>>>>>>> main
		convey.So(err, convey.ShouldBeNil)
	})
}
...
<<<<<<< HEAD
		pu, err := getParameterUnit("test/system_vars_config.toml", nil, txnClient)
		if err != nil {
			t.Error(err)
		}
		setGlobalPu(pu)
...
>>>>>>> main
	...
	checkWant := func(ses, existSes, newSesAfterSession *Session,
		...
		//same session
		v1_val, err := ses.GetSessionVar(nil, v)
		convey.So(err, convey.ShouldBeNil)
		...
		//exist session
		v2_val, err := existSes.GetSessionVar(nil, v)
		convey.So(err, convey.ShouldBeNil)
		...
		v3_val, err := newSesAfterSession.GetSessionVar(nil, v)
		convey.So(err, convey.ShouldBeNil)
		...
		v6_val, err := newSesAfterSession.GetGlobalVar(nil, v)
		convey.So(err, convey.ShouldBeNil)
	...
	checkWant2 := func(ses, existSes, newSesAfterSession *Session,
		...
		_, err = existSes.GetGlobalVar(nil, v)
		convey.So(err, convey.ShouldNotBeNil)
		convey.So(err, convey.ShouldBeError, moerr.NewInternalError(context.TODO(), errorSystemVariableSessionEmpty()))
		...
		//new session after session global
		_, err = newSesAfterSession.GetGlobalVar(nil, v)
		convey.So(err, convey.ShouldNotBeNil)
		convey.So(err, convey.ShouldBeError, moerr.NewInternalError(context.TODO(), errorSystemVariableSessionEmpty()))
	...
	convey.Convey("scope global", t, func() {
		...
		v1_want := 10
		err := ses.SetSessionVar(ctx, v1, v1_want)
		convey.So(err, convey.ShouldNotBeNil)
		...
		err = ses.SetGlobalVar(ctx, v1, v1_want)
		convey.So(err, convey.ShouldBeNil)
		...
		v2_want := 10
		err = ses.SetSessionVar(ctx, v2, v2_want)
		convey.So(err, convey.ShouldNotBeNil)
		...
		err = ses.SetGlobalVar(nil, v2, v2_want)
		convey.So(err, convey.ShouldNotBeNil)
	...
	convey.Convey("scope session", t, func() {
		...
		err = ses.SetGlobalVar(ctx, v2, v2_want)
		convey.So(err, convey.ShouldNotBeNil)
		newSes4 := genSession(ctrl, gSysVars)
	...
	convey.Convey("scope both - set session", t, func() {
		...
		err = ses.SetSessionVar(ctx, v2, 10)
		convey.So(err, convey.ShouldNotBeNil)

		err = ses.SetGlobalVar(ctx, v2, 10)
		convey.So(err, convey.ShouldNotBeNil)
	})
	...
	convey.Convey("scope both", t, func() {
		...
		err := ses.SetGlobalVar(nil, v1, v1_want)
		convey.So(err, convey.ShouldBeNil)
...
func Test_doSelectGlobalSystemVariable(t *testing.T) {
	convey.Convey("select global system variable fail", t, func() {
		...
		_, err := ses.getGlobalSystemVariableValue(nil, "autocommit")
		convey.So(err, convey.ShouldNotBeNil)
	})
...
>>>>>>> main
	...
	err := s.Migrate(s.GetTxnHandler().GetTxnCtx(), &query.MigrateConnToRequest{
		DB: "d1",
